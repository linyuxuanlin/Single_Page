...
- 实时库线风险方向提示：预测将触碰左/右/后哪条库线时，屏幕对应边缘提前按距离分级脉冲预警；实际触线升级为红色高亮。视觉状态直接消费 `coach.mjs` 的精确 `hitLines`，不另建近似碰撞规则。
...
- `replay-highlight.mjs`：把回放碰撞信息转换成稳定的库线/车身高亮状态，支持触线库线标签和脉冲强度，为主 Three.js 场景的红色闪烁与车身轮廓强调提供单一数据源。
...
重点回归覆盖坐标与 WASD 语义、前后可逆、Ackermann、SAT 压线、参考轨迹、实时教练、实时库线风险分级、评分、异常时间戳恢复、严格回放点数预算、关键事件导航、3D 姿态恢复、复盘暂停计时，以及左/右/后库线诊断。`risk-overlay-tests.mjs` 验证非法/重复库线过滤、当前触线、0.8 m 危险阈值、2.0 m 预警阈值以及服务端无 DOM 环境安全退化；`replay-highlight-tests.mjs` 额外验证重复/非法库线过滤、多库线标签、首次触线事件回退提示和高亮样式边界。

网页模块可单独做语法检查：

```bash
node --check risk-overlay.mjs
