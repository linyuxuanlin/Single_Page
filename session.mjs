...
export function recordTrainingSample(session,sample){
  ...
  s.elapsedSec=t;
  s.samples.push({t,rearX:finite(state.rearX),rearZ:finite(state.rearZ),yaw:finite(state.yaw),speed:finite(state.speed),steer,gear:state.gear??null,lateral:finite(deviation.lateral),headingErrorDeg:finite(deviation.headingErrorDeg),lineTouch,parkingSuccess:Boolean(sample.parkingSuccess),coachCode:sample.coachCode??null});
...
export function scoreTrainingMetrics(metrics){const penalties={lineTouch:Math.min(50,finite(metrics.lineTouchEvents)*25),lateral:Math.min(22,Math.max(0,finite(metrics.maxLateralM)-.18)*22),heading:Math.min(16,Math.max(0,finite(metrics.maxHeadingErrorDeg)-5)*.8),speed:Math.min(8,Math.max(0,finite(metrics.maxSpeedKmh)-4.5)*2.5),steering:Math.min(10,Math.max(0,finite(metrics.steeringDirectionChanges)-5)*1.5),incomplete:metrics.completed?0:12};const totalPenalty=Object.values(penalties).reduce((a,b)=>a+b,0);return{score:Math.max(0,Math.round(100-totalPenalty)),penalties,totalPenalty};}
