...
const sample=(t,x,{touch=false,steer=.2,gear='R',lat=0,heading=0,speed=-.4,success=false,yaw=0}={})=>({t,state:{rearX:x,rearZ:-x*.5,yaw,speed,steer,gear},deviation:{lateral:lat,headingErrorDeg:heading},lineTouch:touch,parkingSuccess:success});
...
assert.ok(markers.some(m=>m.type==='line-touch'&&m.index===7));assert.ok(markers.some(m=>m.type==='max-lateral'&&m.index===6));assert.ok(markers.some(m=>m.type==='max-heading'&&m.index===8));assert.ok(markers.some(m=>m.type==='max-speed'&&m.index===9));assert.ok(markers.some(m=>m.type==='completion'&&m.index===13));assert.ok(markers.every(m=>typeof m.label==='string'&&m.label.length));
const tiny=buildReplayModel(s,{maxPoints:3,maxMarkers:8});for(const marker of tiny.markers)assert.ok(tiny.trajectory.some(p=>p.index===marker.index),`marker ${marker.type} pose must survive`);assert.deepEqual(tiny.trajectory.map(p=>p.index),[...tiny.trajectory.map(p=>p.index)].sort((a,b)=>a-b));assert.equal(tiny.durationSec,13);
