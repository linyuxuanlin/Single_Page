...
// With only one or two attempts, aggregate coaching must stay in sample-collection mode.
// A single bad run belongs in that run's review, not in a misleading "recent trend" claim.
h=[historyEntry(s(40,false,{lineTouchEvents:3,maxLateralM:.9,maxHeadingErrorDeg:25}),{at:0})];let earlyAdvice=progressAdvice(h).join(' ');assert.match(earlyAdvice,/再完成 2 次练习/);assert.doesNotMatch(earlyAdvice,/最近完成率|最近仍较常触线|最近横向偏差|最近车身角度|稳定/);
h=[historyEntry(s(40,false,{lineTouchEvents:3,maxLateralM:.9,maxHeadingErrorDeg:25}),{at:0}),historyEntry(s(90,true),{at:1})];earlyAdvice=progressAdvice(h).join(' ');assert.match(earlyAdvice,/再完成 1 次练习/);assert.doesNotMatch(earlyAdvice,/最近完成率|最近仍较常触线|最近横向偏差|最近车身角度|稳定/);
